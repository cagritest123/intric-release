...
- Docker image storage: At least 2GB of disk space for Docker images
...
- Vector embeddings require approximately 25x the size of the original text data. For example, 1MB of text documents will result in roughly 25MB of vector data.
- Docker images: Frontend (~150MB) and Backend (~800MB) images require approximately 1GB of storage space
- Additional storage for Docker volumes: Allow at least 10GB of free space for database growth

### Version Compatibility

For production deployments, it's important to maintain version consistency between components:
...
When upgrading, always update the frontend and backend to matching versions. The worker service uses the same image as the backend, ensuring compatibility as long as both are updated together.
...
6. **Verify connectivity**:
   ...
   ```
   You should receive a 200 OK response from the frontend. For the backend, a response (even a 401 Unauthorized) indicates the service is running. You can also check container logs to verify services are functioning properly:
   ```bash
